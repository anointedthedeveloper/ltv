...
export default function ChannelsDirectoryPage() {
  ...
  const countries = ['All', 'US', 'UK', 'Nigeria', 'South Africa', 'France', 'Qatar', 'Japan'] as const;
  ...
  const getCountryFlag = (country: string) => {
    ...
    return flags[country] || country.substring(0, 2);
  };
      ...
      <header className="sticky top-0 z-50 bg-white/80 dark:bg-neutral-950/80 backdrop-blur-xl border-b border-neutral-200 dark:border-neutral-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          ...
          <nav className="flex gap-8 items-center">
            ...
            <Link href="/channels" className="text-neutral-900 dark:text-white font-semibold hover:text-red-600 dark:hover:text-red-500 transition-colors">
              Directory
            </Link>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-4xl font-bold">Channel Directory</h1>
              <p className="text-neutral-600 dark:text-neutral-400">Browse all available channels</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <p className="text-sm font-semibold text-neutral-600 dark:text-neutral-400">Category</p>
        ...
        {/* Country Filter */}
        <div className="space-y-2">
          <p className="text-sm font-semibold text-neutral-600 dark:text-neutral-400">Country</p>
          <div className="flex gap-2 flex-wrap">
        ...
        {filteredChannels.length > 0 ? (
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-neutral-100 dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-800">
                  <th className="px-6 py-4 text-left text-sm font-semibold text-neutral-900 dark:text-white">Channel</th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-neutral-900 dark:text-white">Category</th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-neutral-900 dark:text-white">Country</th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-neutral-900 dark:text-white">Action</th>
                </tr>
              </thead>
              <tbody>
                {filteredChannels.map((channel) => (
                  <tr key={channel.id} className="border-b border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <span className="text-2xl">{channel.icon || '📺'}</span>
                        <div className="font-medium text-neutral-900 dark:text-white">{channel.name}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-neutral-600 dark:text-neutral-400">{channel.category}</td>
          ...
          <div className="text-center py-16">
            ...
            <p className="text-neutral-500 dark:text-neutral-500 text-sm">Try adjusting your filters or search terms</p>
          </div>
        )}
      </main>
    </div>
